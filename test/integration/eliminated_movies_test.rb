...
class EliminatedMoviesTest < ActionDispatch::IntegrationTest
  ...
  test "As a User, I want to delete movies" do
     ...
     # Setup Movies Data
    user   = users(:one)
    # movies = Movie.in(country_code:"PE").playing_now.remove(user.movies.ids)
    movie  = movies(:one)
    ...
    # Open Modal and click trash icon
    within(".modal") do
      assert has_button?("delete-movie-button"), 
        "Modal should have 'delete movie' button"
      click_button("delete-movie-button")
    end

    # assert_not has_link?(movie.id), 
    #   "Movie Poster should dissapear from CARTELERA after 'deleting' movie"

    assert_includes user.eliminated_movies, EliminatedMovie.where(movie_id: movie.id, user_id: user.id).first, 
      "User.eliminated_movies should contain deleted movie"
  end
end
